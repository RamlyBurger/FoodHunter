    ...
    <div class="row justify-content-center">
        <div class="col-md-5 col-lg-4">
            <div class="text-center mb-4">
                <div class="d-inline-flex align-items-center justify-content-center rounded-circle mb-3" style="width: 80px; height: 80px; background: linear-gradient(135deg, var(--success-color), #30D158);">
                    <i class="bi bi-envelope-check text-white fs-2"></i>
                </div>
                <h2 style="font-weight: 700; letter-spacing: -0.5px;">Verify Your Email</h2>
                <p class="text-muted">We've sent a 6-digit code to<br><strong>{{ $email }}</strong></p>
            </div>

            <div id="email-status" class="alert alert-info mb-3 d-none">
                <i class="bi bi-hourglass-split me-2"></i> <span id="email-status-text">Sending verification code...</span>
            </div>
            ...
            <div class="card">
                <div class="card-body p-4">
                    ...
                    <div class="text-center">
                        ...
                        <button type="button" class="btn btn-link p-0 fw-semibold" id="resend-btn" style="color: var(--primary-color);" onclick="resendCode()">Resend Code</button>
                    </div>
                </div>
            </div>

            <div class="text-center mt-4">
                <a href="{{ route('register') }}" class="text-muted text-decoration-none d-inline-flex align-items-center gap-1">
...
(function() {
    ...
    async function sendOtpEmail(email, isResend = false) {
        ...
        try {
            ...
            if (error) {
                ...
                statusText.innerHTML = '<i class="bi bi-check-circle me-2"></i> Verification code sent to your email!';
                otpSent = true;
                startCountdown();
            ...
            statusEl.classList.remove('alert-info');
            statusEl.classList.add('alert-danger');
            statusText.innerHTML = '<i class="bi bi-exclamation-circle me-2"></i> Failed to send email. Please click Resend Code.';
        }
    }
